...
for basin_shp in in_shps:
    ...
    nc_file = list(in_nc_dir.glob('obs_trmm_pr_*.nc'))
    if len(nc_file) == 0:
        continue
    nc_file = nc_file[0]
    ...
    out_df = out_ds[obs_info['var_name']].to_dataframe().reset_index().groupby([obs_info['lat_name'], obs_info['lon_name']]).filter(lambda x: not all(x[obs_info['var_name']].isna()))
    out_df.rename(columns={obs_info['lat_name']: 'lat', obs_info['lon_name']: 'lon', obs_info['var_name']: 'precip'}, inplace=True)
    out_df['src'] = obs_name
