...
for basin_shp in in_shps:
    print('Processing {} --> tave'.format(basin_shp.parent.name))
    basin_gdf = salem.read_shapefile(basin_shp)     # read shp

    out_xlsx_file_name = out_xls_dir / '{}_{}.xlsx'.format(basin_shp.parent.name, VAR2)
    ...
    out_df.rename(columns={
        obs_info['var_name']: VAR2
    }, inplace=True)

    out_df[['lon', 'lat', 'year', 'month', 'day', 'src', VAR2]].to_excel(out_xlsx, XLS_SHEET_NAME)
    out_xlsx.save()
    out_xlsx.close()
